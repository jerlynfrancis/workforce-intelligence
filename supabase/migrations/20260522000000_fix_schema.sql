DROP TABLE IF EXISTS public.shift_pattern_summary CASCADE;
DROP TABLE IF EXISTS public.weekly_workforce CASCADE;
DROP TABLE IF EXISTS public.daily_operations CASCADE;
DROP TABLE IF EXISTS public.site_master CASCADE;

CREATE TABLE public.site_master (
    ...
    budgeted_weekly_hours INTEGER,
    budgeted_headcount INTEGER,
    is_agency_heavy BOOLEAN DEFAULT false,
    is_overstaffed_low_demand BOOLEAN DEFAULT false,
    is_hidden_cost BOOLEAN DEFAULT false
);
...
CREATE TABLE public.shift_pattern_summary (
    site_id INTEGER NOT NULL REFERENCES public.site_master(site_id),
    week_start_date DATE NOT NULL,
...
DO $$ BEGIN
    CREATE POLICY "Allow public insert access" ON public.site_master
        FOR INSERT WITH CHECK (true);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
    CREATE POLICY "Allow public insert access" ON public.daily_operations
        FOR INSERT WITH CHECK (true);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
    CREATE POLICY "Allow public insert access" ON public.shift_pattern_summary
        FOR INSERT WITH CHECK (true);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

GRANT SELECT, INSERT ON public.site_master TO anon, authenticated, authenticator;
GRANT SELECT, INSERT ON public.daily_operations TO anon, authenticated, authenticator;
GRANT SELECT, INSERT ON public.weekly_workforce TO anon, authenticated, authenticator;
GRANT SELECT, INSERT ON public.shift_pattern_summary TO anon, authenticated, authenticator;
